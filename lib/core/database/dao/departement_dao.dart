...
class DepartementDao {
  DepartementDao({SQLiteService? sqlite}) : _sqlite = sqlite ?? SQLiteService();
  ...
  Future<int> update(String id, Map<String, dynamic> data) async {
    final db = await _sqlite.db;
    return db.update(DbTables.departements, data, where: 'id = ?', whereArgs: [id]);
  }

  Future<int> delete(String id) async {
    final db = await _sqlite.db;
    return db.delete(DbTables.departements, where: 'id = ?', whereArgs: [id]);
  }

  Future<Map<String, dynamic>?> getById(String id) async {
    final db = await _sqlite.db;
    final rows = await db.query(DbTables.departements, where: 'id = ?', whereArgs: [id], limit: 1);
